...
def main():
    ...
    """while True:
        if token_available:  # É a vez do jogador/dealer
            if player.index == game.state['dealer']:
                dealer_action(player, game, socket_sender, NEXT_NODE_ADDRESS)  # Dealer joga
            #else:
            #    player_action(player, game, socket_sender, NEXT_NODE_ADDRESS)  # Jogador joga
            token_available = False
        else:
            network_communication = ring_messages(
                CURRENT_NODE_ADDRESS, NEXT_NODE_ADDRESS, game, socket_receiver, socket_sender, player
            )
            if network_communication == 1:  # Recebe o token
                token_available = True
            elif network_communication == 0:  # Jogo acabou
                break
    """
