...
iniciaAlocador:
    pushq %rbp                # registro de ativação
    movq %rsp, %rbp          
    ...
    popq %rbp             #restauração do registro de ativação
    ret                                         
...
finalizaAlocador:
    ...
    movq $12, %rax            # %rax <- 12 (syscall brk)
    movq HEAP_START, %rdi     # %rdi <- HEAP_START (parametro), num_bytes = %rdi
    syscall                                     
...
exit_inside_alloc_if:
    ...
    movq -24(%rbp), %r10        # %r10 <- freeSpace
    movq HEAP_END, %r10           # freeSpace <- HEAP_START
    ...
    movq HEAP_END, %rcx      # %rcx <- HEAP_START
...
exit_aloc_if:
    ...
    addq $40, %rsp          # restaurando o espaço das 5 variaveis locais
    popq %rbp                                   
    ret   
...
end_print_while:
    ...
    addq $32, %rsp          # restaurando o espaço para 4 variaveis locais
    popq %rbp                    
    ret   
